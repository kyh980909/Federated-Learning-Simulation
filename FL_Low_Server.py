...
with open('low_group_list.txt', 'r') as f:
    low_ue_list = ast.literal_eval(f.read())

low_group_global_accuracy = []  # 각 레이어 들의 가중치가 평균보다 높은 UE 그룹의 정확도
...
for round in range(100):  # Communication Round, Global epoch
    ...
    wandb.log({'low group global accuracy': low_group_global_accuracy[round],
               'low group global loss': low_group_global_loss[round], 'global epoch': round})
